...
export class BlogObserver extends ApiNavigateableObserver {
  ...
  processRequestResponse(response) {
    ...
    if (this.subject.stateStore.Blog.posts.length != 0) {
      this.subject.stateStore.Blog.posts = []; // clear the posts we have so we don't get duplicates
    }
    console.log("clear stale posts, get all posts without duplicates, meh");
